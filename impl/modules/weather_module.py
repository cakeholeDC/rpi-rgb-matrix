...
class WeatherModule:
    def __init__(self, config):
        ...
        if config is not None and 'OWM' in config and 'token' in config['OWM'] and config['OWM']['token'] != "" and 'lat' in config['OWM'] and 'lon' in config['OWM']:   
            self.owm = OWM(config['OWM']['token'])
